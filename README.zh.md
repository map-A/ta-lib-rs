...
[English](README.md) | [API 文档](https://docs.rs/polars-ta-core) | [Wiki](https://github.com/map-A/ta-lib-rs/wiki)

用纯 Rust 实现的完整 ta-lib 技术分析库，包含全部 158 个指标：

- 🦀 **纯 Rust** — 无 C 依赖，安全代码
- ⚡ **高性能** — 大多数指标优于或持平 ta-lib
- 🐼 **Polars 集成** — 一流的 Series/DataFrame API
- 🤖 **AI 友好** — 完善的文档，便于 LLM 辅助开发
- ✅ **黄金测试** — 684/688 个测试用例通过 ta-lib 参考实现验证
...
cd ta-lib-rs
./scripts/run_golden.sh     # 运行黄金测试（对比 ta-lib）
./scripts/bench_all.sh      # 运行完整性能对比
```

## 性能对比

所有基准测试在 n=10,000 数据点上运行（µs = 微秒，中位延迟）。  
比率 = polars-ta / ta-lib（越低越好；`N/A` 表示 ta-lib 未提供该指标的 Python 基准）。
...
- [指标参考（英文）](docs/INDICATORS.md)
- [指标参考（中文）](docs/INDICATORS.zh.md)
- [AI 使用指南（英文）](docs/AI_GUIDE.md)
- [自定义指标指南（英文）](docs/CUSTOM_INDICATOR.md)
...
ta-lib-rs/
├── crates/
│   ├── polars-ta-core/     # 核心 Rust 实现（无 Polars 依赖）
│   ├── polars-ta/          # Polars Series/DataFrame 封装
│   ├── polars-ta-plugin/   # PyO3/Polars Python 插件
│   └── polars-ta-verify/   # 黄金测试和基准测试
├── scripts/                # 实用脚本
├── docs/                   # 文档
└── golden/                 # 黄金测试用例（JSON 格式）
```

## 贡献

参见 [docs/CUSTOM_INDICATOR.md](docs/CUSTOM_INDICATOR.md) 了解如何添加新指标。
