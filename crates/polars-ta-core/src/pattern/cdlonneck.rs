...
//! Bearish continuation: bearish candle + bullish candle that closes at/near previous low.
use super::helpers::*;

pub fn cdlonneck(open: &[f64], high: &[f64], low: &[f64], close: &[f64]) -> Vec<f64> {
    let n = open.len();
    ...
    for i in lookback..n {
        ...
        let is_pattern =
            ...
            open[i] < low[i-1] &&
            (close[i] - low[i-1]).abs() <= avg_equal * EQUAL_FACTOR;
