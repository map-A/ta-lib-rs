//! CDLHAMMER — Hammer
//! Small body near top, long lower shadow, short upper shadow. Bullish reversal.
use super::helpers::*;
...
pub fn cdlhammer(open: &[f64], high: &[f64], low: &[f64], close: &[f64]) -> Vec<f64> {
    ...
    for i in (period + 1)..n {
        ...
        let is_pattern =
            ...
            // body is near the high: open and close are both near the high
            open[i].min(close[i]) >= high[i] - avg_near * NEAR_FACTOR &&
            // Prior candle is bearish (downtrend confirmation)
