...
pub fn cdladvanceblock(open: &[f64], high: &[f64], low: &[f64], close: &[f64]) -> Vec<f64> {
    ...
    let shadows = |i: usize| upper_shadow(open[i], high[i], close[i]) + lower_shadow(open[i], low[i], close[i]);
    let mut ss: [f64; 3] = [
        (ts..lookback).map(|i| shadows(i)).sum(),     // [0]: bars[2..11]
    ...
    let mut near: [f64; 3] = [
        0.0,
        (tn..lookback).map(|i| hl_range(high[i-1], low[i-1])).sum(),  // [1]: bars[6..10]
        (tn..lookback).map(|i| hl_range(high[i-2], low[i-2])).sum(),  // [2]: bars[5..9]
    ];
    let mut far: [f64; 3] = [
        0.0,
        (tn..lookback).map(|i| hl_range(high[i-1], low[i-1])).sum(),
        (tn..lookback).map(|i| hl_range(high[i-2], low[i-2])).sum(),
    ];
    // BodyLong: trailing = lookback - body_p = 2, init bars [i-2] for i=2..11
    let tb = lookback - body_p; // = 2
    let mut body_sum: f64 = (tb..lookback).map(|i| real_body(open[i-2], close[i-2])).sum();
    ...
    for i in lookback..n {
        ...
        let avg_sl0 = real_body(open[i],   close[i])   * SHADOW_VERY_LONG_FACTOR / 2.0; // factor=1.0, ShadowLong
        ...
        for k in 1..3usize {
            near[k] += hl_range(high[i-k],  low[i-k])  - hl_range(high[n_trail.wrapping_sub(k)],  low[n_trail.wrapping_sub(k)]);
            far[k]  += hl_range(high[i-k],  low[i-k])  - hl_range(high[f_trail.wrapping_sub(k)],  low[f_trail.wrapping_sub(k)]);
        }
        body_sum += real_body(open[i-2], close[i-2]) - real_body(open[b_trail.wrapping_sub(2)], close[b_trail.wrapping_sub(2)]);
        ss_trail += 1;
        n_trail  += 1;
        f_trail  += 1;
        b_trail  += 1;
