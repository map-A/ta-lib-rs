...
pub fn cdlgravestonedoji(open: &[f64], high: &[f64], low: &[f64], close: &[f64]) -> Vec<f64> {
    let n = open.len();
    ...
    for i in lookback..n {
        ...
        let is_pattern =
            ...
            upper_shadow(open[i], high[i], close[i]) > avg_body * SHADOW_LONG_FACTOR;

        if is_pattern { out[i] = -100.0; }  // ta-lib returns -100 for gravestone doji
