...
pub fn cdlseparatinglines(open: &[f64], high: &[f64], low: &[f64], close: &[f64]) -> Vec<f64> {
    ...
    let mut body_sum: f64 = (1..=body_period)
    ...
    let mut equal_sum: f64 = ((lookback - equal_period)..lookback)
        .map(|j| hl_range(high[j - 1], low[j - 1]))
        .sum();
    ...
    for i in lookback..n {
        ...
        let belt_hold = (c1 == 1
            ...
            || (c1 == -1
                && upper_shadow(open[i], high[i], close[i])
                    < avg_shadow * SHADOW_VERY_SHORT_FACTOR);
        ...
        shadow_sum +=
            hl_range(high[i], low[i]) - hl_range(high[shadow_trailing], low[shadow_trailing]);
        shadow_trailing += 1;
        body_sum +=
            real_body(open[i], close[i]) - real_body(open[body_trailing], close[body_trailing]);
        body_trailing += 1;
        equal_sum += hl_range(high[i - 1], low[i - 1])
            - hl_range(high[equal_trailing - 1], low[equal_trailing - 1]);
        equal_trailing += 1;
    }
    out
}
