...
pub fn cdlkicking(open: &[f64], high: &[f64], low: &[f64], close: &[f64]) -> Vec<f64> {
    ...
    let mut body_prev: f64   = (0..BODY_LONG_PERIOD).map(|j| real_body(open[j], close[j])).sum();
    let mut vshort_prev: f64 = (0..SHADOW_VERY_SHORT_PERIOD).map(|j| hl_range(high[j], low[j])).sum();
    let mut prev_trail = 0usize;
    // Curr (anchor i): TrailingIdx = startIdx-PERIOD = 1, init [1..10]
    let mut body_curr: f64   = (1..=BODY_LONG_PERIOD).map(|j| real_body(open[j], close[j])).sum();
    let mut vshort_curr: f64 = (1..=SHADOW_VERY_SHORT_PERIOD).map(|j| hl_range(high[j], low[j])).sum();
    ...
    for i in lookback..n {
        ...
        let maru_prev = real_body(open[i-1], close[i-1]) > avg_body_p * BODY_LONG_FACTOR
            && upper_shadow(open[i-1], high[i-1], close[i-1]) < avg_vshort_p * SHADOW_VERY_SHORT_FACTOR
            && lower_shadow(open[i-1], low[i-1],  close[i-1]) < avg_vshort_p * SHADOW_VERY_SHORT_FACTOR;
        ...
        if c0 != c1 && maru_prev && maru_curr {
            if c0 == -1 && gap_up   { out[i] =  100.0; }
            if c0 ==  1 && gap_down { out[i] = -100.0; }
        }

        body_prev   += real_body(open[i-1], close[i-1]) - real_body(open[prev_trail], close[prev_trail]);
        vshort_prev += hl_range(high[i-1], low[i-1]) - hl_range(high[prev_trail], low[prev_trail]);
        prev_trail  += 1;
