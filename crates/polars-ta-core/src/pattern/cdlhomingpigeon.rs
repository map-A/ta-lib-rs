...
pub fn cdlhomingpigeon(open: &[f64], _high: &[f64], _low: &[f64], close: &[f64]) -> Vec<f64> {
    ...
    let mut body_short_sum: f64 = (1..=BODY_SHORT_PERIOD).map(|j| real_body(open[j], close[j])).sum();
    ...
    for i in lookback..n {
        ...
        let is_pattern =
            candle_color(open[i-1], close[i-1]) == -1 &&               // 1st black
        ...
        body_short_sum += real_body(open[i], close[i]) - real_body(open[short_trail], close[short_trail]);
        short_trail += 1;
    }
    out
}
