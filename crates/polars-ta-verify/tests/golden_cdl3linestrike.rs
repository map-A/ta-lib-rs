//! Golden tests for CDL3LINESTRIKE.
use polars_ta_core::pattern::cdl3linestrike;
...
fn run_cdl3linestrike_golden(filename: &str, epsilon: f64) {
    ...
    let actual = cdl3linestrike(&open, &high, &low, &close);
    let label = format!("cdl3linestrike/{}", golden.meta.dataset);
    assert_close(&actual, golden.get_output_values("values").unwrap(), epsilon, &label);
}

#[test]
fn cdl3linestrike_normal_1000() { run_cdl3linestrike_golden("cdl3linestrike__normal_1000.json", 1.0); }
