//! Golden tests for CDLTRISTAR.
use polars_ta_core::pattern::cdltristar;
...
fn run_cdltristar_golden(filename: &str, epsilon: f64) {
    ...
    let actual = cdltristar(&open, &high, &low, &close);
    let label = format!("cdltristar/{}", golden.meta.dataset);
    assert_close(&actual, golden.get_output_values("values").unwrap(), epsilon, &label);
}

#[test]
fn cdltristar_normal_1000() { run_cdltristar_golden("cdltristar__normal_1000.json", 1.0); }
