//! Golden tests for CDLIDENTICAL3CROWS.
use polars_ta_core::pattern::cdlidentical3crows;
...
fn run_cdlidentical3crows_golden(filename: &str, epsilon: f64) {
    ...
    let actual = cdlidentical3crows(&open, &high, &low, &close);
    let label = format!("cdlidentical3crows/{}", golden.meta.dataset);
    assert_close(&actual, golden.get_output_values("values").unwrap(), epsilon, &label);
}

#[test]
fn cdlidentical3crows_normal_1000() { run_cdlidentical3crows_golden("cdlidentical3crows__normal_1000.json", 1.0); }
