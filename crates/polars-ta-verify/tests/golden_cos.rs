...
#[test]
fn cos_normal_1000() {
    run_cos_golden("cos__normal_1000.json", 1e-10);
}
#[test]
#[ignore = "NaN propagation: ta-lib skips NaN, we propagate per IEEE 754"]
fn cos_with_nan() {
    run_cos_golden("cos__with_nan_5pct.json", 1e-10);
}
#[test]
fn cos_all_same_value() {
    run_cos_golden("cos__all_same_value.json", 1e-10);
}
#[test]
fn cos_real_btcusdt() {
    run_cos_golden("cos__real_btcusdt_1d.json", 1e-10);
}
#[test]
fn cos_real_flat_period() {
    run_cos_golden("cos__real_flat_period.json", 1e-10);
}
