//! Golden tests for CDLGRAVESTONEDOJI.
use polars_ta_core::pattern::cdlgravestonedoji;
...
fn run_cdlgravestonedoji_golden(filename: &str, epsilon: f64) {
    ...
    let actual = cdlgravestonedoji(&open, &high, &low, &close);
    let label = format!("cdlgravestonedoji/{}", golden.meta.dataset);
    assert_close(&actual, golden.get_output_values("values").unwrap(), epsilon, &label);
}

#[test]
fn cdlgravestonedoji_normal_1000() { run_cdlgravestonedoji_golden("cdlgravestonedoji__normal_1000.json", 1.0); }
