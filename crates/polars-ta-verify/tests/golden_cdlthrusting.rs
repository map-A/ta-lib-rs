//! Golden tests for CDLTHRUSTING.
use polars_ta_core::pattern::cdlthrusting;
...
fn run_cdlthrusting_golden(filename: &str, epsilon: f64) {
    ...
    let actual = cdlthrusting(&open, &high, &low, &close);
    let label = format!("cdlthrusting/{}", golden.meta.dataset);
    assert_close(&actual, golden.get_output_values("values").unwrap(), epsilon, &label);
}

#[test]
fn cdlthrusting_normal_1000() { run_cdlthrusting_golden("cdlthrusting__normal_1000.json", 1.0); }
