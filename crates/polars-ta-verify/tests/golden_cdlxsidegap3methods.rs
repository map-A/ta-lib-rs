//! Golden tests for CDLXSIDEGAP3METHODS.
use polars_ta_core::pattern::cdlxsidegap3methods;
...
fn run_cdlxsidegap3methods_golden(filename: &str, epsilon: f64) {
    ...
    let actual = cdlxsidegap3methods(&open, &high, &low, &close);
    let label = format!("cdlxsidegap3methods/{}", golden.meta.dataset);
    assert_close(&actual, golden.get_output_values("values").unwrap(), epsilon, &label);
}

#[test]
fn cdlxsidegap3methods_normal_1000() { run_cdlxsidegap3methods_golden("cdlxsidegap3methods__normal_1000.json", 1.0); }
