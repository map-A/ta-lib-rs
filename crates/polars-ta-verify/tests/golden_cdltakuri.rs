//! Golden tests for CDLTAKURI.
use polars_ta_core::pattern::cdltakuri;
...
fn run_cdltakuri_golden(filename: &str, epsilon: f64) {
    ...
    let actual = cdltakuri(&open, &high, &low, &close);
    let label = format!("cdltakuri/{}", golden.meta.dataset);
    assert_close(&actual, golden.get_output_values("values").unwrap(), epsilon, &label);
}

#[test]
fn cdltakuri_normal_1000() { run_cdltakuri_golden("cdltakuri__normal_1000.json", 1.0); }
