//! Golden tests for CDLSPINNINGTOP.
use polars_ta_core::pattern::cdlspinningtop;
...
fn run_cdlspinningtop_golden(filename: &str, epsilon: f64) {
    ...
    let actual = cdlspinningtop(&open, &high, &low, &close);
    let label = format!("cdlspinningtop/{}", golden.meta.dataset);
    assert_close(&actual, golden.get_output_values("values").unwrap(), epsilon, &label);
}

#[test]
fn cdlspinningtop_normal_1000() { run_cdlspinningtop_golden("cdlspinningtop__normal_1000.json", 1.0); }
