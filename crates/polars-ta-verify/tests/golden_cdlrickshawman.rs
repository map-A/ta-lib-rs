//! Golden tests for CDLRICKSHAWMAN.
use polars_ta_core::pattern::cdlrickshawman;
...
fn run_cdlrickshawman_golden(filename: &str, epsilon: f64) {
    ...
    let actual = cdlrickshawman(&open, &high, &low, &close);
    let label = format!("cdlrickshawman/{}", golden.meta.dataset);
    assert_close(&actual, golden.get_output_values("values").unwrap(), epsilon, &label);
}

#[test]
fn cdlrickshawman_normal_1000() { run_cdlrickshawman_golden("cdlrickshawman__normal_1000.json", 1.0); }
