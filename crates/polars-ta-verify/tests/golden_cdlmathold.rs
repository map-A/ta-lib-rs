//! Golden tests for CDLMATHOLD.
use polars_ta_core::pattern::cdlmathold;
...
fn run_cdlmathold_golden(filename: &str, epsilon: f64) {
    ...
    let actual = cdlmathold(&open, &high, &low, &close);
    let label = format!("cdlmathold/{}", golden.meta.dataset);
    assert_close(&actual, golden.get_output_values("values").unwrap(), epsilon, &label);
}

#[test]
fn cdlmathold_normal_1000() { run_cdlmathold_golden("cdlmathold__normal_1000.json", 1.0); }
