//! Golden tests for CDLHAMMER.
use polars_ta_core::pattern::cdlhammer;
...
fn run_cdlhammer_golden(filename: &str, epsilon: f64) {
    ...
    let actual = cdlhammer(&open, &high, &low, &close);
    let label = format!("cdlhammer/{}", golden.meta.dataset);
    assert_close(&actual, golden.get_output_values("values").unwrap(), epsilon, &label);
}

#[test]
fn cdlhammer_normal_1000() { run_cdlhammer_golden("cdlhammer__normal_1000.json", 1.0); }
