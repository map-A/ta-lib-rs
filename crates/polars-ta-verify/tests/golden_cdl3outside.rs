//! Golden tests for CDL3OUTSIDE.
use polars_ta_core::pattern::cdl3outside;
...
fn run_cdl3outside_golden(filename: &str, epsilon: f64) {
    ...
    let actual = cdl3outside(&open, &high, &low, &close);
    let label = format!("cdl3outside/{}", golden.meta.dataset);
    assert_close(&actual, golden.get_output_values("values").unwrap(), epsilon, &label);
}

#[test]
fn cdl3outside_normal_1000() { run_cdl3outside_golden("cdl3outside__normal_1000.json", 1.0); }
