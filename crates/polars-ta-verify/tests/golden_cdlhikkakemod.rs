//! Golden tests for CDLHIKKAKEMOD.
use polars_ta_core::pattern::cdlhikkakemod;
...
fn run_cdlhikkakemod_golden(filename: &str, epsilon: f64) {
    ...
    let actual = cdlhikkakemod(&open, &high, &low, &close);
    let label = format!("cdlhikkakemod/{}", golden.meta.dataset);
    assert_close(&actual, golden.get_output_values("values").unwrap(), epsilon, &label);
}

#[test]
fn cdlhikkakemod_normal_1000() { run_cdlhikkakemod_golden("cdlhikkakemod__normal_1000.json", 1.0); }
