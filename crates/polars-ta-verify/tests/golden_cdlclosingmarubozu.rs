//! Golden tests for CDLCLOSINGMARUBOZU.
use polars_ta_core::pattern::cdlclosingmarubozu;
...
fn run_cdlclosingmarubozu_golden(filename: &str, epsilon: f64) {
    ...
    let actual = cdlclosingmarubozu(&open, &high, &low, &close);
    let label = format!("cdlclosingmarubozu/{}", golden.meta.dataset);
    assert_close(&actual, golden.get_output_values("values").unwrap(), epsilon, &label);
}

#[test]
fn cdlclosingmarubozu_normal_1000() { run_cdlclosingmarubozu_golden("cdlclosingmarubozu__normal_1000.json", 1.0); }
