//! Golden tests for CDLMARUBOZU.
use polars_ta_core::pattern::cdlmarubozu;
...
fn run_cdlmarubozu_golden(filename: &str, epsilon: f64) {
    ...
    let actual = cdlmarubozu(&open, &high, &low, &close);
    let label = format!("cdlmarubozu/{}", golden.meta.dataset);
    assert_close(&actual, golden.get_output_values("values").unwrap(), epsilon, &label);
}

#[test]
fn cdlmarubozu_normal_1000() { run_cdlmarubozu_golden("cdlmarubozu__normal_1000.json", 1.0); }
