...
fn run_var_golden(filename: &str, period: usize, nbdev: f64, epsilon: f64) {
    ...
    let label = format!("var(period={},nbdev={})/{}", period, nbdev, golden.meta.dataset);
    assert_close(&actual, golden.get_output_values("values").unwrap(), epsilon, &label);
}

#[test]
fn var_period5_normal_1000() { run_var_golden("var_period5_nbdev1.0_normal_1000.json", 5, 1.0, 1e-8); }

#[test]
fn var_period5_boundary_exact() { run_var_golden("var_period5_nbdev1.0_boundary_exact.json", 5, 1.0, 1e-8); }
...
fn var_period5_with_nan() { run_var_golden("var_period5_nbdev1.0_with_nan_5pct.json", 5, 1.0, 1e-8); }

#[test]
fn var_period5_all_same_value() { run_var_golden("var_period5_nbdev1.0_all_same_value.json", 5, 1.0, 1e-6); }
...
#[test]
fn var_period5_real_flat_period() { run_var_golden("var_period5_nbdev1.0_real_flat_period.json", 5, 1.0, 1e-8); }
