//! Golden tests for CDLDRAGONFLYDOJI.
use polars_ta_core::pattern::cdldragonflydoji;
...
fn run_cdldragonflydoji_golden(filename: &str, epsilon: f64) {
    ...
    let actual = cdldragonflydoji(&open, &high, &low, &close);
    let label = format!("cdldragonflydoji/{}", golden.meta.dataset);
    assert_close(&actual, golden.get_output_values("values").unwrap(), epsilon, &label);
}

#[test]
fn cdldragonflydoji_normal_1000() { run_cdldragonflydoji_golden("cdldragonflydoji__normal_1000.json", 1.0); }
