//! Golden tests for CDLONNECK.
use polars_ta_core::pattern::cdlonneck;
...
fn run_cdlonneck_golden(filename: &str, epsilon: f64) {
    ...
    let actual = cdlonneck(&open, &high, &low, &close);
    let label = format!("cdlonneck/{}", golden.meta.dataset);
    assert_close(&actual, golden.get_output_values("values").unwrap(), epsilon, &label);
}

#[test]
fn cdlonneck_normal_1000() { run_cdlonneck_golden("cdlonneck__normal_1000.json", 1.0); }
