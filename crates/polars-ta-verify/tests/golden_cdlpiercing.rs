//! Golden tests for CDLPIERCING.
use polars_ta_core::pattern::cdlpiercing;
...
fn run_cdlpiercing_golden(filename: &str, epsilon: f64) {
    ...
    let actual = cdlpiercing(&open, &high, &low, &close);
    let label = format!("cdlpiercing/{}", golden.meta.dataset);
    assert_close(&actual, golden.get_output_values("values").unwrap(), epsilon, &label);
}

#[test]
fn cdlpiercing_normal_1000() { run_cdlpiercing_golden("cdlpiercing__normal_1000.json", 1.0); }
