//! Golden tests for CDLDARKCLOUDCOVER.
use polars_ta_core::pattern::cdldarkcloudcover;
...
fn run_cdldarkcloudcover_golden(filename: &str, epsilon: f64) {
    ...
    let actual = cdldarkcloudcover(&open, &high, &low, &close);
    let label = format!("cdldarkcloudcover/{}", golden.meta.dataset);
    assert_close(&actual, golden.get_output_values("values").unwrap(), epsilon, &label);
}

#[test]
fn cdldarkcloudcover_normal_1000() { run_cdldarkcloudcover_golden("cdldarkcloudcover__normal_1000.json", 1.0); }
