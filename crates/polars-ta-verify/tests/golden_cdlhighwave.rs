//! Golden tests for CDLHIGHWAVE.
use polars_ta_core::pattern::cdlhighwave;
...
fn run_cdlhighwave_golden(filename: &str, epsilon: f64) {
    ...
    let actual = cdlhighwave(&open, &high, &low, &close);
    let label = format!("cdlhighwave/{}", golden.meta.dataset);
    assert_close(&actual, golden.get_output_values("values").unwrap(), epsilon, &label);
}

#[test]
fn cdlhighwave_normal_1000() { run_cdlhighwave_golden("cdlhighwave__normal_1000.json", 1.0); }
