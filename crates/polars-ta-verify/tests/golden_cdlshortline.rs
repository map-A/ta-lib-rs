//! Golden tests for CDLSHORTLINE.
use polars_ta_core::pattern::cdlshortline;
...
fn run_cdlshortline_golden(filename: &str, epsilon: f64) {
    ...
    let actual = cdlshortline(&open, &high, &low, &close);
    let label = format!("cdlshortline/{}", golden.meta.dataset);
    assert_close(&actual, golden.get_output_values("values").unwrap(), epsilon, &label);
}

#[test]
fn cdlshortline_normal_1000() { run_cdlshortline_golden("cdlshortline__normal_1000.json", 1.0); }
