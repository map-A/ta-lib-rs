//! Golden tests for CDLMORNINGSTAR.
use polars_ta_core::pattern::cdlmorningstar;
...
fn run_cdlmorningstar_golden(filename: &str, epsilon: f64) {
    ...
    let actual = cdlmorningstar(&open, &high, &low, &close);
    let label = format!("cdlmorningstar/{}", golden.meta.dataset);
    assert_close(&actual, golden.get_output_values("values").unwrap(), epsilon, &label);
}

#[test]
fn cdlmorningstar_normal_1000() { run_cdlmorningstar_golden("cdlmorningstar__normal_1000.json", 1.0); }
