//! Golden tests for CDLKICKINGBYLENGTH.
use polars_ta_core::pattern::cdlkickingbylength;
...
fn run_cdlkickingbylength_golden(filename: &str, epsilon: f64) {
    ...
    let actual = cdlkickingbylength(&open, &high, &low, &close);
    let label = format!("cdlkickingbylength/{}", golden.meta.dataset);
    assert_close(&actual, golden.get_output_values("values").unwrap(), epsilon, &label);
}

#[test]
fn cdlkickingbylength_normal_1000() { run_cdlkickingbylength_golden("cdlkickingbylength__normal_1000.json", 1.0); }
