//! Golden tests for CDLHARAMICROSS.
use polars_ta_core::pattern::cdlharamicross;
...
fn run_cdlharamicross_golden(filename: &str, epsilon: f64) {
    ...
    let actual = cdlharamicross(&open, &high, &low, &close);
    let label = format!("cdlharamicross/{}", golden.meta.dataset);
    assert_close(&actual, golden.get_output_values("values").unwrap(), epsilon, &label);
}

#[test]
fn cdlharamicross_normal_1000() { run_cdlharamicross_golden("cdlharamicross__normal_1000.json", 1.0); }
