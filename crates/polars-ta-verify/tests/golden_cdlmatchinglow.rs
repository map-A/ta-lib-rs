//! Golden tests for CDLMATCHINGLOW.
use polars_ta_core::pattern::cdlmatchinglow;
...
fn run_cdlmatchinglow_golden(filename: &str, epsilon: f64) {
    ...
    let actual = cdlmatchinglow(&open, &high, &low, &close);
    let label = format!("cdlmatchinglow/{}", golden.meta.dataset);
    assert_close(&actual, golden.get_output_values("values").unwrap(), epsilon, &label);
}

#[test]
fn cdlmatchinglow_normal_1000() { run_cdlmatchinglow_golden("cdlmatchinglow__normal_1000.json", 1.0); }
