//! Golden tests for CDLDOJI.
use polars_ta_core::pattern::cdldoji;
...
fn run_cdldoji_golden(filename: &str, epsilon: f64) {
    ...
    let actual = cdldoji(&open, &high, &low, &close);
    let label = format!("cdldoji/{}", golden.meta.dataset);
    assert_close(&actual, golden.get_output_values("values").unwrap(), epsilon, &label);
}

#[test]
fn cdldoji_normal_1000() { run_cdldoji_golden("cdldoji__normal_1000.json", 1.0); }
