//! Golden tests for CDLADVANCEBLOCK.
use polars_ta_core::pattern::cdladvanceblock;
...
fn run_cdladvanceblock_golden(filename: &str, epsilon: f64) {
    ...
    let actual = cdladvanceblock(&open, &high, &low, &close);
    let label = format!("cdladvanceblock/{}", golden.meta.dataset);
    assert_close(&actual, golden.get_output_values("values").unwrap(), epsilon, &label);
}

#[test]
fn cdladvanceblock_normal_1000() { run_cdladvanceblock_golden("cdladvanceblock__normal_1000.json", 1.0); }
