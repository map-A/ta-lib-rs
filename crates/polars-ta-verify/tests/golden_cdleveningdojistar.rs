//! Golden tests for CDLEVENINGDOJISTAR.
use polars_ta_core::pattern::cdleveningdojistar;
...
fn run_cdleveningdojistar_golden(filename: &str, epsilon: f64) {
    ...
    let actual = cdleveningdojistar(&open, &high, &low, &close);
    let label = format!("cdleveningdojistar/{}", golden.meta.dataset);
    assert_close(&actual, golden.get_output_values("values").unwrap(), epsilon, &label);
}

#[test]
fn cdleveningdojistar_normal_1000() { run_cdleveningdojistar_golden("cdleveningdojistar__normal_1000.json", 1.0); }
