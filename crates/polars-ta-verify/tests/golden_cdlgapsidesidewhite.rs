//! Golden tests for CDLGAPSIDESIDEWHITE.
use polars_ta_core::pattern::cdlgapsidesidewhite;
...
fn run_cdlgapsidesidewhite_golden(filename: &str, epsilon: f64) {
    ...
    let actual = cdlgapsidesidewhite(&open, &high, &low, &close);
    let label = format!("cdlgapsidesidewhite/{}", golden.meta.dataset);
    assert_close(&actual, golden.get_output_values("values").unwrap(), epsilon, &label);
}

#[test]
fn cdlgapsidesidewhite_normal_1000() { run_cdlgapsidesidewhite_golden("cdlgapsidesidewhite__normal_1000.json", 1.0); }
