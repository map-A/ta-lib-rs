//! Golden tests for CDLSTALLEDPATTERN.
use polars_ta_core::pattern::cdlstalledpattern;
...
fn run_cdlstalledpattern_golden(filename: &str, epsilon: f64) {
    ...
    let actual = cdlstalledpattern(&open, &high, &low, &close);
    let label = format!("cdlstalledpattern/{}", golden.meta.dataset);
    assert_close(&actual, golden.get_output_values("values").unwrap(), epsilon, &label);
}

#[test]
fn cdlstalledpattern_normal_1000() { run_cdlstalledpattern_golden("cdlstalledpattern__normal_1000.json", 1.0); }
