//! Golden tests for CDL3INSIDE.
use polars_ta_core::pattern::cdl3inside;
...
fn run_cdl3inside_golden(filename: &str, epsilon: f64) {
    ...
    let actual = cdl3inside(&open, &high, &low, &close);
    let label = format!("cdl3inside/{}", golden.meta.dataset);
    assert_close(&actual, golden.get_output_values("values").unwrap(), epsilon, &label);
}

#[test]
fn cdl3inside_normal_1000() { run_cdl3inside_golden("cdl3inside__normal_1000.json", 1.0); }
