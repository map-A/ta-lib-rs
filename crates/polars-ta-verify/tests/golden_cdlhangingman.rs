//! Golden tests for CDLHANGINGMAN.
use polars_ta_core::pattern::cdlhangingman;
...
fn run_cdlhangingman_golden(filename: &str, epsilon: f64) {
    ...
    let actual = cdlhangingman(&open, &high, &low, &close);
    let label = format!("cdlhangingman/{}", golden.meta.dataset);
    assert_close(&actual, golden.get_output_values("values").unwrap(), epsilon, &label);
}

#[test]
fn cdlhangingman_normal_1000() { run_cdlhangingman_golden("cdlhangingman__normal_1000.json", 1.0); }
