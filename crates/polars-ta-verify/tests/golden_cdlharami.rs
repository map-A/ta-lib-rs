//! Golden tests for CDLHARAMI.
use polars_ta_core::pattern::cdlharami;
...
fn run_cdlharami_golden(filename: &str, epsilon: f64) {
    ...
    let actual = cdlharami(&open, &high, &low, &close);
    let label = format!("cdlharami/{}", golden.meta.dataset);
    assert_close(&actual, golden.get_output_values("values").unwrap(), epsilon, &label);
}

#[test]
fn cdlharami_normal_1000() { run_cdlharami_golden("cdlharami__normal_1000.json", 1.0); }
