//! Golden tests for CDLHIKKAKE.
use polars_ta_core::pattern::cdlhikkake;
...
fn run_cdlhikkake_golden(filename: &str, epsilon: f64) {
    ...
    let actual = cdlhikkake(&open, &high, &low, &close);
    let label = format!("cdlhikkake/{}", golden.meta.dataset);
    assert_close(&actual, golden.get_output_values("values").unwrap(), epsilon, &label);
}

#[test]
fn cdlhikkake_normal_1000() { run_cdlhikkake_golden("cdlhikkake__normal_1000.json", 1.0); }
