//! Golden tests for CDLDOJISTAR.
use polars_ta_core::pattern::cdldojistar;
...
fn run_cdldojistar_golden(filename: &str, epsilon: f64) {
    ...
    let actual = cdldojistar(&open, &high, &low, &close);
    let label = format!("cdldojistar/{}", golden.meta.dataset);
    assert_close(&actual, golden.get_output_values("values").unwrap(), epsilon, &label);
}

#[test]
fn cdldojistar_normal_1000() { run_cdldojistar_golden("cdldojistar__normal_1000.json", 1.0); }
