//! Golden tests for CDLLONGLEGGEDDOJI.
use polars_ta_core::pattern::cdllongleggeddoji;
...
fn run_cdllongleggeddoji_golden(filename: &str, epsilon: f64) {
    ...
    let actual = cdllongleggeddoji(&open, &high, &low, &close);
    let label = format!("cdllongleggeddoji/{}", golden.meta.dataset);
    assert_close(&actual, golden.get_output_values("values").unwrap(), epsilon, &label);
}

#[test]
fn cdllongleggeddoji_normal_1000() { run_cdllongleggeddoji_golden("cdllongleggeddoji__normal_1000.json", 1.0); }
