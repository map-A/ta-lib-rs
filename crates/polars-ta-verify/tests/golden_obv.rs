...
#[test]
fn obv_normal_1000() {
    run_obv_golden("obv__normal_1000.json", 1e-7);
}
#[test]
#[ignore = "ta-lib NaN propagation differs for OBV with NaN inputs"]
fn obv_with_nan() {
    run_obv_golden("obv__with_nan_5pct.json", 1e-10);
}
#[test]
fn obv_all_same_value() {
    run_obv_golden("obv__all_same_value.json", 1e-10);
}
#[test]
fn obv_real_btcusdt() {
    run_obv_golden("obv__real_btcusdt_1d.json", 1e-3);
}
#[test]
fn obv_real_flat_period() {
    run_obv_golden("obv__real_flat_period.json", 1e-8);
}
