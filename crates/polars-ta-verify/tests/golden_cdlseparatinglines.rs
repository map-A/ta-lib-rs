//! Golden tests for CDLSEPARATINGLINES.
use polars_ta_core::pattern::cdlseparatinglines;
...
fn run_cdlseparatinglines_golden(filename: &str, epsilon: f64) {
    ...
    let actual = cdlseparatinglines(&open, &high, &low, &close);
    let label = format!("cdlseparatinglines/{}", golden.meta.dataset);
    assert_close(&actual, golden.get_output_values("values").unwrap(), epsilon, &label);
}

#[test]
fn cdlseparatinglines_normal_1000() { run_cdlseparatinglines_golden("cdlseparatinglines__normal_1000.json", 1.0); }
