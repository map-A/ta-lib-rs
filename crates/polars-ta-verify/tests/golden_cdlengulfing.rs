//! Golden tests for CDLENGULFING.
use polars_ta_core::pattern::cdlengulfing;
...
fn run_cdlengulfing_golden(filename: &str, epsilon: f64) {
    ...
    let actual = cdlengulfing(&open, &high, &low, &close);
    let label = format!("cdlengulfing/{}", golden.meta.dataset);
    assert_close(&actual, golden.get_output_values("values").unwrap(), epsilon, &label);
}

#[test]
fn cdlengulfing_normal_1000() { run_cdlengulfing_golden("cdlengulfing__normal_1000.json", 1.0); }
