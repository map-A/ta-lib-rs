//! Golden tests for CDLLADDERBOTTOM.
use polars_ta_core::pattern::cdlladderbottom;
...
fn run_cdlladderbottom_golden(filename: &str, epsilon: f64) {
    ...
    let actual = cdlladderbottom(&open, &high, &low, &close);
    let label = format!("cdlladderbottom/{}", golden.meta.dataset);
    assert_close(&actual, golden.get_output_values("values").unwrap(), epsilon, &label);
}

#[test]
fn cdlladderbottom_normal_1000() { run_cdlladderbottom_golden("cdlladderbottom__normal_1000.json", 1.0); }
