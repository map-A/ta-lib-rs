//! Golden tests for CDLINVERTEDHAMMER.
use polars_ta_core::pattern::cdlinvertedhammer;
...
fn run_cdlinvertedhammer_golden(filename: &str, epsilon: f64) {
    ...
    let actual = cdlinvertedhammer(&open, &high, &low, &close);
    let label = format!("cdlinvertedhammer/{}", golden.meta.dataset);
    assert_close(&actual, golden.get_output_values("values").unwrap(), epsilon, &label);
}

#[test]
fn cdlinvertedhammer_normal_1000() { run_cdlinvertedhammer_golden("cdlinvertedhammer__normal_1000.json", 1.0); }
