//! Golden tests for CDLABANDONEDBABY.
use polars_ta_core::pattern::cdlabandonedbaby;
...
fn run_cdlabandonedbaby_golden(filename: &str, epsilon: f64) {
    ...
    let actual = cdlabandonedbaby(&open, &high, &low, &close);
    let label = format!("cdlabandonedbaby/{}", golden.meta.dataset);
    assert_close(&actual, golden.get_output_values("values").unwrap(), epsilon, &label);
}

#[test]
fn cdlabandonedbaby_normal_1000() { run_cdlabandonedbaby_golden("cdlabandonedbaby__normal_1000.json", 1.0); }
