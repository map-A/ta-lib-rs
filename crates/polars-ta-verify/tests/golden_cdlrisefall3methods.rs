//! Golden tests for CDLRISEFALL3METHODS.
use polars_ta_core::pattern::cdlrisefall3methods;
...
fn run_cdlrisefall3methods_golden(filename: &str, epsilon: f64) {
    ...
    let actual = cdlrisefall3methods(&open, &high, &low, &close);
    let label = format!("cdlrisefall3methods/{}", golden.meta.dataset);
    assert_close(&actual, golden.get_output_values("values").unwrap(), epsilon, &label);
}

#[test]
fn cdlrisefall3methods_normal_1000() { run_cdlrisefall3methods_golden("cdlrisefall3methods__normal_1000.json", 1.0); }
