//! Golden tests for CDLCONCEALBABYSWALL.
use polars_ta_core::pattern::cdlconcealbabyswall;
...
fn run_cdlconcealbabyswall_golden(filename: &str, epsilon: f64) {
    ...
    let actual = cdlconcealbabyswall(&open, &high, &low, &close);
    let label = format!("cdlconcealbabyswall/{}", golden.meta.dataset);
    assert_close(&actual, golden.get_output_values("values").unwrap(), epsilon, &label);
}

#[test]
fn cdlconcealbabyswall_normal_1000() { run_cdlconcealbabyswall_golden("cdlconcealbabyswall__normal_1000.json", 1.0); }
