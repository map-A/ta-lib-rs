//! Golden tests for CDLHOMINGPIGEON.
use polars_ta_core::pattern::cdlhomingpigeon;
...
fn run_cdlhomingpigeon_golden(filename: &str, epsilon: f64) {
    ...
    let actual = cdlhomingpigeon(&open, &high, &low, &close);
    let label = format!("cdlhomingpigeon/{}", golden.meta.dataset);
    assert_close(&actual, golden.get_output_values("values").unwrap(), epsilon, &label);
}

#[test]
fn cdlhomingpigeon_normal_1000() { run_cdlhomingpigeon_golden("cdlhomingpigeon__normal_1000.json", 1.0); }
