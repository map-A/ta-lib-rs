//! Golden tests for CDLCOUNTERATTACK.
use polars_ta_core::pattern::cdlcounterattack;
...
fn run_cdlcounterattack_golden(filename: &str, epsilon: f64) {
    ...
    let actual = cdlcounterattack(&open, &high, &low, &close);
    let label = format!("cdlcounterattack/{}", golden.meta.dataset);
    assert_close(&actual, golden.get_output_values("values").unwrap(), epsilon, &label);
}

#[test]
fn cdlcounterattack_normal_1000() { run_cdlcounterattack_golden("cdlcounterattack__normal_1000.json", 1.0); }
