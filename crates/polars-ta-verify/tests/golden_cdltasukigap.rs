//! Golden tests for CDLTASUKIGAP.
use polars_ta_core::pattern::cdltasukigap;
...
fn run_cdltasukigap_golden(filename: &str, epsilon: f64) {
    ...
    let actual = cdltasukigap(&open, &high, &low, &close);
    let label = format!("cdltasukigap/{}", golden.meta.dataset);
    assert_close(&actual, golden.get_output_values("values").unwrap(), epsilon, &label);
}

#[test]
fn cdltasukigap_normal_1000() { run_cdltasukigap_golden("cdltasukigap__normal_1000.json", 1.0); }
