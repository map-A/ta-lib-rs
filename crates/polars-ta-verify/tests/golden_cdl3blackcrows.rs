//! Golden tests for CDL3BLACKCROWS.
use polars_ta_core::pattern::cdl3blackcrows;
...
fn run_cdl3blackcrows_golden(filename: &str, epsilon: f64) {
    ...
    let actual = cdl3blackcrows(&open, &high, &low, &close);
    let label = format!("cdl3blackcrows/{}", golden.meta.dataset);
    assert_close(&actual, golden.get_output_values("values").unwrap(), epsilon, &label);
}

#[test]
fn cdl3blackcrows_normal_1000() { run_cdl3blackcrows_golden("cdl3blackcrows__normal_1000.json", 1.0); }
