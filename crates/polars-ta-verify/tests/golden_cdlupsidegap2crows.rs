//! Golden tests for CDLUPSIDEGAP2CROWS.
use polars_ta_core::pattern::cdlupsidegap2crows;
...
fn run_cdlupsidegap2crows_golden(filename: &str, epsilon: f64) {
    ...
    let actual = cdlupsidegap2crows(&open, &high, &low, &close);
    let label = format!("cdlupsidegap2crows/{}", golden.meta.dataset);
    assert_close(&actual, golden.get_output_values("values").unwrap(), epsilon, &label);
}

#[test]
fn cdlupsidegap2crows_normal_1000() { run_cdlupsidegap2crows_golden("cdlupsidegap2crows__normal_1000.json", 1.0); }
