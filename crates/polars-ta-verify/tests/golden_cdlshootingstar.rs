//! Golden tests for CDLSHOOTINGSTAR.
use polars_ta_core::pattern::cdlshootingstar;
...
fn run_cdlshootingstar_golden(filename: &str, epsilon: f64) {
    ...
    let actual = cdlshootingstar(&open, &high, &low, &close);
    let label = format!("cdlshootingstar/{}", golden.meta.dataset);
    assert_close(&actual, golden.get_output_values("values").unwrap(), epsilon, &label);
}

#[test]
fn cdlshootingstar_normal_1000() { run_cdlshootingstar_golden("cdlshootingstar__normal_1000.json", 1.0); }
