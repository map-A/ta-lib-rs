//! Golden tests for CDLBELTHOLD.
use polars_ta_core::pattern::cdlbelthold;
...
fn run_cdlbelthold_golden(filename: &str, epsilon: f64) {
    ...
    let actual = cdlbelthold(&open, &high, &low, &close);
    let label = format!("cdlbelthold/{}", golden.meta.dataset);
    assert_close(&actual, golden.get_output_values("values").unwrap(), epsilon, &label);
}

#[test]
fn cdlbelthold_normal_1000() { run_cdlbelthold_golden("cdlbelthold__normal_1000.json", 1.0); }
