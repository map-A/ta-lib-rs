//! Golden tests for CDLINNECK.
use polars_ta_core::pattern::cdlinneck;
...
fn run_cdlinneck_golden(filename: &str, epsilon: f64) {
    ...
    let actual = cdlinneck(&open, &high, &low, &close);
    let label = format!("cdlinneck/{}", golden.meta.dataset);
    assert_close(&actual, golden.get_output_values("values").unwrap(), epsilon, &label);
}

#[test]
fn cdlinneck_normal_1000() { run_cdlinneck_golden("cdlinneck__normal_1000.json", 1.0); }
