//! Golden tests for CDLSTICKSANDWICH.
use polars_ta_core::pattern::cdlsticksandwich;
...
fn run_cdlsticksandwich_golden(filename: &str, epsilon: f64) {
    ...
    let actual = cdlsticksandwich(&open, &high, &low, &close);
    let label = format!("cdlsticksandwich/{}", golden.meta.dataset);
    assert_close(&actual, golden.get_output_values("values").unwrap(), epsilon, &label);
}

#[test]
fn cdlsticksandwich_normal_1000() { run_cdlsticksandwich_golden("cdlsticksandwich__normal_1000.json", 1.0); }
